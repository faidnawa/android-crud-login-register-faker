...
class DB_Functions {
    ...
    public function cekemail($email,$token) {
	    ...
	    public function resetpass($to,$saltup,$pasup) {
        ...
        if ($stmt->execute()) {
            ...
            if ($cek == $to) {
                //cek pass
				if ($user){
				$stmt = $this->conn->prepare("UPDATE users SET salt = ? , encrypted_password = ? WHERE token = ?");
				$stmt->bind_param("sss", $saltup, $pasup, $to);
				  $result = $stmt->execute();
				$stmt->close();
				if ($result){
				 // cek status
